...
public class Exercise1 {
    ...
    public List<String> iteratingOverKeys(){
        ...
        for(String var:countries.keySet()){
            if(var.startsWith("R")){
                seekingCountries.add(var);
            }
        }
    ...
    public List<String> iteratingOverEntries(){
        ...
        for(String var:countries.keySet()){
             if( (var.contains("q")) || (var.contains("Q"))){
                String var2=var.toLowerCase();
                seekingCountries.add(var2);
            }
        }
        return seekingCountries;
    }
    ...
    public String iteratingOverValues(){
        ...
        for(String var:countries.keySet()){
            String capitala=countries.get(var);
            if(capitala.length()>seekingCapital.length()){
